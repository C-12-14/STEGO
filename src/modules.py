...
# Extending the dino model for our use
class DinoFeaturizer(nn.Module):
    def __init__(self, dim, cfg):
        ...
        for p in self.model.parameters():
            p.requires_grad = (
                False  # We do not want to train the dino model during our training
            )
        self.model.eval().cuda()  # We will be running the model in evaluation mode and not in training mode
        self.dropout = torch.nn.Dropout2d(p=0.1)

        # Deciding the url for the required version of DINO
        ...
        # self.n_feats are the number of features that are in the output of the DINO model
        # self.dim are the number of features we will be using in STEGO
        if arch == "vit_small":
            self.n_feats = 384
        else:
            self.n_feats = 768
    ...
    # Conv layer to transform the self.n_feats features into self.dim number of features
    ...
    def make_nonlinear_clusterer(self, in_channels):
        return torch.nn.Sequential(
            ...
            torch.nn.ReLU(),  # Adding non-linearity
    ...
    def forward(self, img, n=1, return_class_feat=False):
        self.model.eval()  # Using the model in evaluation mode
        with torch.no_grad():
